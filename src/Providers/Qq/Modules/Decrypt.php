 ...
 * QQ 小程序客户端敏感数据解密
 *
 * 用于解密 getPhoneNumber 等接口经客户端回传的 encryptedData。
 * QQ 与微信采用同一套对称算法（AES-128-CBC + PKCS#7 + watermark），
 * 底层实现见 {@see Aes128CbcPkcs7}。
 *
 * 安全约束：
 *   - 解密后的 watermark.appid 必须与当前 QQ 小程序 appId 一致，否则视为伪造数据。
    ...
    public function phone(string $encryptedData, string $sessionKey, string $iv): array
    {
        ...
        foreach (['phoneNumber', 'purePhoneNumber', 'countryCode'] as $key) {
            if (!isset($data[$key]) || !is_string($data[$key])) {
                throw new ApiException("QQ 手机号解密结果缺少字段：{$key}", -1);
            }
        }
